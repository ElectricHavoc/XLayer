...
namespace XLayer.Decoder
{
    internal class MpegFrame : FrameBase, IMpegFrame
    {
        ...
        internal int GetSideDataSize()
        {
            switch (Layer)
            {
                case MpegLayer.LayerI:
                    if (ChannelMode == MpegChannelMode.Mono) return 16;
                    else if (ChannelMode == MpegChannelMode.Stereo || ChannelMode == MpegChannelMode.DualChannel) return 32;
                    else
                    {
                        // joint stereo...  ugh...
                ...
                case MpegLayer.LayerIII:
                    if (ChannelMode == MpegChannelMode.Mono && Version >= MpegVersion.Version2) return 9;
                    else if (ChannelMode != MpegChannelMode.Mono && Version < MpegVersion.Version2) return 32;
                    else return 17;
            }
            return 0;
        }
        ...
        internal VBRInfo ParseVBR()
        {
            ...
            {
                MpegVersion.Version1 when ChannelMode != MpegChannelMode.Mono => 36,
                > MpegVersion.Version1 when ChannelMode == MpegChannelMode.Mono => 13,
                _ => 21
            };
            ...
            if (buf[0..4].SequenceEqual("Xing"u8) || buf[0..4].SequenceEqual("Info"u8))
            {
                return ParseXing(offset + 4);
            }

            if (Read(36, buf) != 4) return null;
            if (buf[0..4].SequenceEqual("VBRI"u8))
            {
                return ParseVBRI();
            }
